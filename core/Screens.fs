...
module MessagesScreen =
    ...
    type Msg = MessagesLoaded of Message []

    let init userName =
        { messages = [||] },
        S.get
        |> A.exec' (
            UserMessages.selectMessageForUser userName
            >> MessagesLoaded
        )
...
module ThreadsScreen =
    ...
    let init =
        { threads = [||]
          ...
          notAuthorized = false },
        S.get
        |> A.exec' (getMessages >> LocalThreadsLoaded)
    ...
    let update model =
        function
        | LocalThreadsLoaded (Some threads) -> { model with threads = threads }, syncThreads None
        | LocalThreadsLoaded None -> { model with notAuthorized = true }, syncThreads None
        | NextPageLoaded (Ok (threads, None)) -> { model with threads = threads }, Effect.none
        | NextPageLoaded (Ok (threads, (Some _ as next))) ->
            { model with
                  threads = threads
                  pageLoaded = model.pageLoaded + 1 },
...
module LoginScreen =
    ...
    let private computeIsEnable model =
        { model with
              isEnabled = model.username <> "" && model.password <> "" }
    ...
    let update model msg =
        ...
        | LoginMsg ->
            { model with
                  isBusy = true
                  error = None },
            Domain.login model.username model.password
            |> Effect.map LoginEnd
        ...
        | LoginEnd (Error e) ->
            { model with
                  ...
                  error = Some <| string e },
            Effect.none
        | UsernameMsg x -> { model with username = x } |> computeIsEnable, Effect.none
        | PasswordMsg x -> { model with password = x } |> computeIsEnable, Effect.none
        | _ -> model, Effect.none
...
module ProfileScreen =
    ...
    let private mkUserUrl (db: LocalDb) =
        db.userName |> Option.map UrlBuilder.user

    let init =
        ProfileLoading,
        S.get
        |> S.withSync' mkUserUrl
        |> A.exec (fun db -> db.profile) ProfileLoaded
    ...
    let update model msg =
        ...
        | ProfileLoaded (Ok None) ->
            LoginScreen.init
            |> Effect.merge LoginModel LoginMsg
        | ProfileLoaded (Error e) -> failwithf "ProfileLoaded: %O" e
        | Logout ->
            model,
            S.sync UrlBuilder.logout
            |> S.withPost (always LocalDb.empty)
            |> A.exec (always ()) LogoutEnd
        | LogoutEnd _ -> init
        | LoginMsg (LoginScreen.LoginEnd (Ok _)) -> init
        | LoginMsg childMsg ->
            match model with
            | LoginModel childModel ->
                LoginScreen.update childModel childMsg
                |> Effect.merge LoginModel LoginMsg
            | _ -> model, Effect.none
...
module FeedScreen =
    ...
    let init source =
        { source = source
          ...
          scroll = None },
        S.get
        |> A.exec (fun db -> FeedMerger.getPostsWithLevels source db.feeds) PostsLoadedFromCache

    let private updateFeeds source (db: LocalDb) =
        let (feeds, sharedFeeds, _) =
            FeedMerger.mergeFirstPage source db.sharedFeeds db.feeds
    ...
    let update model msg =
        let toItems (ps: PostsWithLevels) loading : PostState [] =
            if Seq.isEmpty ps.preloaded
               && not ^ Seq.isEmpty ps.actual
               && not loading then
                Array.concat [ ps.actual |> Array.map Actual
                               [| LoadNextDivider |]
                               ps.old |> Array.map Old ]
            else
                Array.concat [ ps.actual |> Array.map Actual
                               ps.old |> Array.map Old ]
        ...
        | FirstPagePreloaded (Ok posts) ->
            if Array.isEmpty posts.actual
               && Array.isEmpty posts.old then
                { model with
                      hasNew = true
                      loading = false },
        ...
        | ApplyPreloaded ->
            ...
            |> S.withPost
                (fun db ->
                    { db with
                          feeds = FeedMerger.mergePreloaded' model.source db.feeds })
            |> A.exec'
                (fun db ->
                    FeedMerger.getPostsWithLevels model.source db.feeds
                    |> ApplyPreloadedCompleted)
        | ApplyPreloadedCompleted xs -> { model with items = toItems xs false }, Effect.none
        | LoadNextPage ->
            ...
            |> S.withSync'
                (fun db ->
                    ...
                    UrlBuilder.posts model.source "FIXME" a.nextPage
                    |> Some)
            |> S.withPost
                (fun db ->
                    let (feeds, sharedFeeds) =
                        FeedMerger.mergeSecondPage model.source db.feeds db.sharedFeeds
        ...
        | Refresh ->
            ...
            |> S.withPost
                (fun db ->
                    let (feeds, sharedFeeds) =
                        FeedMerger.replacePosts model.source db.sharedFeeds db.feeds
...
module TagsScreen =
    ...
    let private topTags (db: LocalDb) =
        db.topTags |> Map.toArray |> Array.map snd

    let private userTags (db: LocalDb) =
        db.userTags |> Map.toArray |> Array.map snd
    ...
    let init =
        Model.empty,
        Effect.batch [ S.get |> A.exec' (topTags >> TopTags)
                       S.get |> A.exec' (userTags >> UserTags)
                       S.get
                       |> S.withSync UrlBuilder.home
                       |> A.exec topTags TopTagsSynced
                       S.get
                       |> S.withSync' mkUserUrl
                       |> A.exec userTags UserTagsSynced ]
...
module PostScreen =
    ...
    let init id =
        { tags = [||]
          ...
          id = id
          comments = [||] },
        Effect.batch [ S.get |> A.exec (getPosts id) PostLoaded
                       S.get
                       |> S.withSync (UrlBuilder.post id)
                       |> A.exec (getPosts id) RefreshComplete ]
    ...
    let private updateModel (model: Model) (post: Post option) =
        let comments =
            ...
            | Some post ->
                post.comments
                |> Array.filter
                   ^ fun comment -> comment.parentId = 0 && comment.rating >= -0.3
                |> Array.sortByDescending
                   ^ fun comment -> comment.rating
    ...
    let update (model: Model) =
        ...
        | RefreshComplete (Ok post) ->
            { (updateModel model post) with
                  error = None },
            Effect.none
...
module ApplicationScreen =
    ...
    let init _ =
        ...
        { history = [ TabsModel m ]
          title = "JoyReactor (0.6.6)" },
        cmd |> Effect.map TabsMsg

    let update model msg =
        match model, msg with
        | { history = _ :: ((_ :: _) as prev) }, NavigateBack -> { model with history = prev }, Effect.none
